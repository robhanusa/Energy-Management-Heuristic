...
# View at address http://127.0.0.1:8050/
        ...
        dcc.Tab(label='Time Lapse', 
            ...
            dbc.Container([
                ...
                dcc.Interval(id="interval",interval=300, 
                              n_intervals=0, max_intervals=len(r2_sx_out)-501)
                ])
            ]),
        
        dcc.Tab(label='Grid Consumption',children=[
            dcc.Graph(figure=fig_grid_cons)
            ])
        ])
    ])
...
def update(n_intervals, start_watch, counter):
    return update_data(n_intervals, start_watch, counter)
